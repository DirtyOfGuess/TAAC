...
class ModelInput(NamedTuple):
    ...
    seq_abs_months: dict   # {domain: tensor [B, L]}
    seq_abs_weekdays: dict  # {domain: tensor [B, L]}
    seq_abs_hours: dict    # {domain: tensor [B, L]}
...
class UserPairTokenizer(nn.Module):
    """Encodes aligned user_int / user_dense multi-value pairs.

    Each configured fid appears in both user_int and user_dense with the same
    length. This module injects the aligned dense value into every id embedding,
    pools each fid, then summarizes all configured fids into one vector.
    """
    ...
    def __init__(
        ...
        dense_feature_entries: List[Tuple[int, int, int]],
        pair_fids: List[int],
        log_fids: Optional[List[int]],
        emb_dim: int,
        d_model: int,
        hidden_mult: int = 4,
        emb_skip_threshold: int = 0,
        ...
        super().__init__()
        if len(int_feature_specs) != len(int_feature_ids):
            raise ValueError(
                "int_feature_specs and int_feature_ids must have the same length"
            )
        ...
        dense_info: Dict[int, Tuple[int, int]] = {
            fid: (int(offset), int(length))
            for fid, offset, length in dense_feature_entries
            if fid > 0
        }

        self.feature_specs: List[Tuple[int, int, int]] = []
        self.dense_offsets: List[int] = []
        self.pair_specs: List[Tuple[int, int, int, int, int]] = []
        raw_embs = []

        for fid in self.pair_fids:
            if fid not in int_info:
                raise KeyError(f"pair fid {fid} not found in user_int schema")
            if fid not in dense_info:
                raise KeyError(f"pair fid {fid} not found in user_dense schema")

            vs, int_offset, int_length = int_info[fid]
            dense_offset, dense_length = dense_info[fid]
            if int_length != dense_length:
                raise ValueError(
                    f"pair fid {fid} has mismatched lengths: "
                    f"user_int length={int_length}, user_dense length={dense_length}"
                )

            self.feature_specs.append((vs, int_offset, int_length))
            self.dense_offsets.append(dense_offset)
            self.pair_specs.append((fid, vs, int_offset, dense_offset, int_length))

            skip = int(vs) <= 0 or (emb_skip_threshold > 0 and int(vs) > emb_skip_threshold)
            if skip:
                raw_embs.append(None)
            else:
                raw_embs.append(nn.Embedding(int(vs) + 1, emb_dim, padding_idx=0))

        self.embs = nn.ModuleList([e for e in raw_embs if e is not None])
        self._emb_index = []
        real_idx = 0
        for e in raw_embs:
            if e is not None:
    ...
    def forward(self, int_feats: torch.Tensor, dense_feats: torch.Tensor) -> torch.Tensor:
        """Return a single pair-summary vector of shape (B, d_model)."""
        B = int_feats.shape[0]
        if not self.pair_fids:
            return dense_feats.new_zeros((B, self.d_model))

        tokens = []
        for i, (fid, _, int_offset, dense_offset, length) in enumerate(self.pair_specs):
            real_idx = self._emb_index[i]
            if real_idx == -1:
                tokens.append(dense_feats.new_zeros((B, self.d_model)))
                continue

            int_vals = int_feats[:, int_offset:int_offset + length].long()
            dense_vals = dense_feats[:, dense_offset:dense_offset + length].float()
            dense_vals = torch.nan_to_num(dense_vals, nan=0.0, posinf=0.0, neginf=0.0)
            if fid in self.log_fids:
                dense_vals = torch.log1p(torch.clamp(dense_vals, min=0.0))
            dense_vals = dense_vals.unsqueeze(-1)

            int_emb = self.embs[real_idx](int_vals)
            pair_pos = int_emb + self.value_proj(dense_vals)
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        abs_time_emb_dim: int = 32,
        # NS tokenizer variant
        ns_tokenizer_type: str = 'rankmixer',
        user_ns_tokens: int = 0,
        item_ns_tokens: int = 0,
        # Optional user_int/user_dense aligned pair branch
        ...
        if isinstance(userpair_fids, str):
            self.userpair_fids = [
                int(fid.strip())
                for fid in userpair_fids.split(',')
                if fid.strip()
            ]
        else:
            self.userpair_fids = [int(fid) for fid in (userpair_fids or [])]
        if isinstance(userpair_log_fids, str):
            self.userpair_log_fids = [
                int(fid.strip())
                for fid in userpair_log_fids.split(',')
                if fid.strip()
            ]
        else:
        ...
        # Optional user pair branch. It fuses aligned user_int/user_dense
        # evidence into the existing user dense token, so T is unchanged.
        self.has_user_pair = len(self.userpair_fids) > 0
        dense_pair_mask = torch.ones(user_dense_dim, dtype=torch.float32)
        if self.has_user_pair:
            if not self.has_user_dense:
                raise ValueError("userpair_fids requires user_dense features")
            if user_int_feature_ids is None or user_dense_feature_entries is None:
                raise ValueError(
                    "userpair_fids was provided but user_int_feature_ids / "
                    "user_dense_feature_entries are missing"
                )
            self.user_pair_tokenizer = UserPairTokenizer(
                int_feature_specs=user_int_feature_specs,
                int_feature_ids=user_int_feature_ids,
                dense_feature_entries=user_dense_feature_entries,
                pair_fids=self.userpair_fids,
            ...
            for fid in self.userpair_fids:
                if fid not in dense_offset_map:
                    raise KeyError(
        ...
        for domain in self.seq_domains:
            ...
            self._seq_proj[domain] = nn.Sequential(
                nn.Linear(len(vs) * emb_dim + 3 * abs_time_emb_dim, d_model),
                nn.LayerNorm(d_model),
            )

        self.abs_month_embedding = nn.Embedding(12 + 1, abs_time_emb_dim, padding_idx=0)
        self.abs_weekday_embedding = nn.Embedding(7 + 1, abs_time_emb_dim, padding_idx=0)
        self.abs_hour_embedding = nn.Embedding(24 + 1, abs_time_emb_dim, padding_idx=0)
    ...
    def _init_params(self) -> None:
        ...
        if self.has_user_pair and self.user_pair_tokenizer is not None:
            for emb in self.user_pair_tokenizer.embs:
                nn.init.xavier_normal_(emb.weight.data)
                emb.weight.data[0, :] = 0

        for emb in [
            self.abs_month_embedding,
    ...
    def reinit_high_cardinality_params(
        ...
        if self.has_user_pair and self.user_pair_tokenizer is not None:
            for i, (vs, offset, length) in enumerate(self.user_pair_tokenizer.feature_specs):
                real_idx = self.user_pair_tokenizer._emb_index[i]
                if real_idx == -1:
                    continue
                emb = self.user_pair_tokenizer.embs[real_idx]
                if int(vs) > cardinality_threshold:
                    nn.init.xavier_normal_(emb.weight.data)
                    emb.weight.data[0, :] = 0
                    reinit_ptrs.add(emb.weight.data_ptr())
        ...
        # Time-related embeddings are always preserved.
        skip_count += 3
        if self.num_time_buckets > 0:
            skip_count += len(self.seq_domains)
    ...
    def _embed_seq_domain(
        ...
        for i in range(S):
            ...
            if real_idx == -1:
                ...
                emb_list.append(e)
        abs_time_emb = torch.cat([
            self.abs_month_embedding(month_ids),
            self.abs_weekday_embedding(weekday_ids),
            self.abs_hour_embedding(hour_ids),
        ], dim=-1)
        cat_emb = torch.cat(emb_list + [abs_time_emb], dim=-1)  # (B, L, S*emb_dim + 3*abs_time_emb_dim)
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        ns_parts = [user_ns]
        if self.has_user_dense:
            dense_input = inputs.user_dense_feats * self.user_dense_pair_mask.to(
        ...
        for domain in self.seq_domains:
            tokens = self._embed_seq_domain(
                ...
                inputs.seq_abs_months[domain],
                inputs.seq_abs_weekdays[domain],
                inputs.seq_abs_hours[domain])
