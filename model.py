...
class RankMixerNSTokenizer(nn.Module):
    ...
    def __init__(
        ...
        # Projection layers for dense-to-weight transformation (one per paired fid)
        if use_weighted_fusion and self.paired_dense_dims:
            self.dense_weight_projs = nn.ModuleDict()
            for fid, dense_dim in self.paired_dense_dims.items():
                # Project dense values to emb_dim for element-wise gating
                self.dense_weight_projs[str(fid)] = nn.Sequential(
                    nn.Linear(dense_dim, emb_dim),
                    nn.Sigmoid(),  # Gate values in [0, 1]
                )
        else:
            self.dense_weight_projs = None

        # Compute total embedding dim
        if use_weighted_fusion:
            # Weighted fusion: dimension stays the same (no concat)
            total_num_fids = sum(len(g) for g in groups)
            total_emb_dim = total_num_fids * emb_dim
        else:
            # Concat fusion: add dense dimensions
            total_num_fids = sum(len(g) for g in groups)
            total_emb_dim = total_num_fids * emb_dim
        ...
        fusion_type = "weighted" if use_weighted_fusion else "concat"
        logging.info(
            f"RankMixerNSTokenizer ({fusion_type}): {total_num_fids} fids, "
    ...
    def forward(
            ...
            dense_schema_entries: Optional[List[Tuple[int, int, int]]] = None,
    ) -> torch.Tensor:
        ...
        Args:
            ...
            dense_feats: (B, total_dense_dim) concatenated dense features.
                Required if there are paired features.
            dense_schema_entries: List of (fid, offset, length) for dense features,
                used to locate paired dense values by fid.

        Returns:
            (B, num_ns_tokens, d_model) tensor.
        """
        # Build dense fid lookup if provided
        dense_fid_map = {}
        if dense_schema_entries is not None:
            for dense_fid, dense_offset, dense_len in dense_schema_entries:
                dense_fid_map[dense_fid] = (dense_offset, dense_len)
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        # Paired dense features config
        user_paired_dense_fids: Optional[List[int]] = None,
        ...
        # Store dense schema for paired feature lookup
        self.user_dense_feature_specs = user_dense_feature_specs
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # 1. NS tokens: grouped projection with paired dense features
        if self.ns_tokenizer_type == 'rankmixer' and self.user_ns_tokenizer.paired_dense_dims:
            # Pass dense features for paired extraction
            user_ns = self.user_ns_tokenizer(
