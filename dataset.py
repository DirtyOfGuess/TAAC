...
SEQ_ABS_TIME_MONTH_SIZE = 12
SEQ_ABS_TIME_WEEKDAY_SIZE = 7
SEQ_ABS_TIME_HOUR_SIZE = 24
CURRENT_TIME_DENSE_DIM = (
    SEQ_ABS_TIME_HOUR_SIZE
    + SEQ_ABS_TIME_WEEKDAY_SIZE
    + SEQ_ABS_TIME_MONTH_SIZE
    + 1
)
...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        self._buf_seq = {}
        self._buf_seq_tb = {}
        self._buf_seq_abs_month = {}
        self._buf_seq_abs_weekday = {}
        self._buf_seq_abs_hour = {}
        self._buf_seq_lens = {}
        for domain in self.seq_domains:
            ...
            self._buf_seq_abs_month[domain] = np.zeros((B, max_len), dtype=np.int64)
            self._buf_seq_abs_weekday[domain] = np.zeros((B, max_len), dtype=np.int64)
            self._buf_seq_abs_hour[domain] = np.zeros((B, max_len), dtype=np.int64)
    ...
    def _load_schema(self, schema_path: str, seq_max_lens: Dict[str, int]) -> None:
        ...
        self._user_dense_time_offset = self.user_dense_schema.total_dim
        for i in range(CURRENT_TIME_DENSE_DIM):
            self.user_dense_schema.add(-(1000 + i), 1)
    ...
    def _fill_abs_time_features(
        self,
        ts_padded: "npt.NDArray[np.int64]",
        month_out: "npt.NDArray[np.int64]",
        weekday_out: "npt.NDArray[np.int64]",
        hour_out: "npt.NDArray[np.int64]",
    ) -> None:
        """Derive absolute month / weekday / hour ids from unix timestamps.

        Timestamps are interpreted in UTC. Outputs are 1-based ids with 0
        reserved for padding so they can be passed directly to embeddings.
        """
        month_out[:] = 0
        weekday_out[:] = 0
        hour_out[:] = 0
        ...
        valid_ts = ts_padded[valid]
        hour_out[valid] = ((valid_ts // 3600) % 24) + 1
        weekday_out[valid] = ((valid_ts // 86400 + 3) % 7) + 1
        month_out[valid] = (
            valid_ts.astype('datetime64[s]').astype('datetime64[M]').astype(np.int64) % 12
        ) + 1

    def _fill_current_time_dense(
        self,
        user_dense: "npt.NDArray[np.float32]",
        timestamps: "npt.NDArray[np.int64]",
    ) -> None:
        """Append one-hot current absolute-time context to user dense features."""
        off = self._user_dense_time_offset
        time_slice = user_dense[:, off:off + CURRENT_TIME_DENSE_DIM]
        time_slice[:] = 0.0

        hours = ((timestamps // 3600) % 24).astype(np.int64)
        weekdays = ((timestamps // 86400 + 3) % 7).astype(np.int64)
        months = (
            timestamps.astype('datetime64[s]').astype('datetime64[M]').astype(np.int64) % 12
        ).astype(np.int64)

        rows = np.arange(user_dense.shape[0])
        user_dense[rows, off + hours] = 1.0
        user_dense[rows, off + SEQ_ABS_TIME_HOUR_SIZE + weekdays] = 1.0
        user_dense[
            rows,
            off + SEQ_ABS_TIME_HOUR_SIZE + SEQ_ABS_TIME_WEEKDAY_SIZE + months,
        ] = 1.0
        user_dense[:, off + CURRENT_TIME_DENSE_DIM - 1] = (weekdays >= 5).astype(np.float32)
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        for ci, dim, offset in self._user_dense_plan:
            ...
            # Apply log transform to large-scale features (fid 62-66)
            # Get the feature id for this column
            fid = self._user_dense_cols[ci][0] if ci < len(self._user_dense_cols) else -1
            if fid in [62, 63, 64, 65, 66]:
                # Log1p transform: log(1 + |x|) * sign(x)
                # This handles negative values and zeros safely
                padded = np.log1p(np.abs(padded)) * np.sign(padded)
            
            user_dense[:, offset:offset + dim] = padded
        self._fill_current_time_dense(user_dense, timestamps)
        ...
        for domain in self.seq_domains:
            ...
            time_bucket[:] = 0
            abs_month = self._buf_seq_abs_month[domain][:B]
            abs_weekday = self._buf_seq_abs_weekday[domain][:B]
            abs_hour = self._buf_seq_abs_hour[domain][:B]
            abs_month[:] = 0
            abs_weekday[:] = 0
            abs_hour[:] = 0
            ...
            result[f'{domain}_abs_month'] = torch.from_numpy(abs_month.copy())
            result[f'{domain}_abs_weekday'] = torch.from_numpy(abs_weekday.copy())
            result[f'{domain}_abs_hour'] = torch.from_numpy(abs_hour.copy())
