...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--abs_time_emb_dim', type=int, default=32,
                        help='Embedding dimension used by each absolute sequence '
                             'time feature (month / weekday / hour).')
    ...
    # ===== 【新增】Paired dense features configuration =====
    parser.add_argument('--user_paired_dense_fids', type=str, default='',
                        help='Comma-separated list of user feature IDs that have '
                             'paired dense features (e.g., "62,63,64,65,66,89,90,91"). '
                             'Empty string means no paired features.')
    parser.add_argument('--use_weighted_fusion', action='store_true', default=True,
                        help='Use weighted fusion for paired int-dense features (default: True). '
                             'If False, use concat fusion instead.')
    parser.add_argument('--no_weighted_fusion', dest='use_weighted_fusion', action='store_false',
                        help='Use concat fusion instead of weighted fusion for paired features.')
...
def main() -> None:
    ...
    model_args = {
        ...
        "abs_time_emb_dim": args.abs_time_emb_dim,
        "ns_tokenizer_type": args.ns_tokenizer_type,
        "user_ns_tokens": args.user_ns_tokens,
        "item_ns_tokens": args.item_ns_tokens,
        # Paired dense features configuration.
        "user_paired_dense_fids": (
            [int(x) for x in args.user_paired_dense_fids.split(',')]
