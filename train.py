...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--userpair_fids', type=str, default='',
                        help='Comma-separated user fid list to fuse as aligned '
                             'user_int/user_dense pairs, e.g. 62,63,64,65,66,89,90,91')
    ...
    if args.userpair_fids:
        args.userpair_fids = [
            int(fid.strip())
            for fid in args.userpair_fids.split(',')
            if fid.strip()
        ]
    else:
        args.userpair_fids = []
    if args.userpair_log_fids:
        args.userpair_log_fids = [
            int(fid.strip())
            for fid in args.userpair_log_fids.split(',')
            if fid.strip()
        ]
    else:
        args.userpair_log_fids = []
...
def main() -> None:
    ...
    user_dense_feature_entries = list(pcvr_dataset.user_dense_schema.entries)
