...
class PCVRHyFormerRankingTrainer:
    ...
    def _build_epoch_dir_name(self, epoch: int) -> str:
        """Build a per-epoch checkpoint sub-directory name such as
        ``epoch3.layer=2.head=4.hidden=64``.
        """
        parts = [f"epoch{epoch}"]
        for key in ("layer", "head", "hidden"):
            if key in self.ckpt_params:
    ...
    def _save_epoch_checkpoint(self, epoch: int) -> str:
        """Save ``model.pt`` plus sidecar files under an ``epoch`` sub-dir."""
        ckpt_dir = os.path.join(self.save_dir, self._build_epoch_dir_name(epoch))
        os.makedirs(ckpt_dir, exist_ok=True)
        torch.save(self.model.state_dict(), os.path.join(ckpt_dir, "model.pt"))
        self._write_sidecar_files(ckpt_dir)
    ...
    def _make_model_input(self, device_batch: Dict[str, Any]) -> ModelInput:
        ...
        seq_abs_months: Dict[str, torch.Tensor] = {}
        seq_abs_weekdays: Dict[str, torch.Tensor] = {}
        seq_abs_hours: Dict[str, torch.Tensor] = {}
        for domain in seq_domains:
            ...
            seq_abs_months[domain] = device_batch.get(
                f'{domain}_abs_month',
                torch.zeros(B, L, dtype=torch.long, device=self.device))
            seq_abs_weekdays[domain] = device_batch.get(
                f'{domain}_abs_weekday',
                torch.zeros(B, L, dtype=torch.long, device=self.device))
            seq_abs_hours[domain] = device_batch.get(
                f'{domain}_abs_hour',
                torch.zeros(B, L, dtype=torch.long, device=self.device))
        return ModelInput(
            ...
            seq_abs_months=seq_abs_months,
            seq_abs_weekdays=seq_abs_weekdays,
            seq_abs_hours=seq_abs_hours,
        )
