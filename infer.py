...
def resolve_model_cfg(train_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key in _MODEL_CFG_KEYS:
        ...
        if key in ('userpair_fids', 'userpair_log_fids'):
            raw = train_config.get(key, _FALLBACK_MODEL_CFG[key])
            if isinstance(raw, str):
                ...
                cfg[key] = [int(fid) for fid in (raw or [])]
            continue
...
def build_model(
    ...
    user_dense_feature_entries = list(dataset.user_dense_schema.entries)
